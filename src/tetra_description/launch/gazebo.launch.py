...
def generate_launch_description():
    model_arg = DeclareLaunchArgument(
        name="model",
        default_value=os.path.join(get_package_share_directory("tetra_description"), "urdf", "simple_dexter_driveTrain.xacro"),
        description="Absolute path to the robot URDF file"
    )

    env_var = SetEnvironmentVariable("GAZEBO_MODEL_PATH",os.path.join(get_package_prefix("tetra_description"), "share"))
    ...
    spawn_robot = Node(
        ...
        arguments=["-entity", "tetra_description", "-topic", "robot_description"],
        output='screen'
    )
